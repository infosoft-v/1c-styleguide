# 1С: Руководство по стилю оформления (вер. 1.2)
...
```bsl
// Хорошо:
строкаОперация = тзВидыОпераций.Добавить();
строкаОперация.сВидОперации         = сВидОперации;
строкаОперация.чГруппаНомер         = ГруппаПоВидуОперации(сВидОперации);
строкаОперация.бПоОрганизацииВЦелом = ПолучитьВидОбработки(сВидОперации);
```
